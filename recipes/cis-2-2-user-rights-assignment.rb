...
security_policy 'Access Credential Manager' do
  policy_template "#{node['secedit']['template']['location']}\\Access Credential Manager.inf}"
...
security_policy 'Access this computer from the network' do
  policy_template "#{node['secedit']['template']['location']}\\Access this computer from the network.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Act as part of the operating system' do
  policy_template "#{node['secedit']['template']['location']}\\Act as part of the operating system.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Adjust memory quotas for a process' do
  policy_template "#{node['secedit']['template']['location']}\\Adjust memory quotas for a process.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Allow log on locally' do
  policy_template "#{node['secedit']['template']['location']}\\Allow log on locally.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Allow log on through Remote Desktop Service' do
  policy_template "#{node['secedit']['template']['location']}\\Allow log on through Remote Desktop Service.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Back up files and directories' do
  policy_template "#{node['secedit']['template']['location']}\\Back up files and directories.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Change the system time' do
  policy_template "#{node['secedit']['template']['location']}\\Change the system time.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Change the time zone' do
  policy_template "#{node['secedit']['template']['location']}\\Change the time zone.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Create a pagefile' do
  policy_template "#{node['secedit']['template']['location']}\\Create a pagefile.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Create a token object' do
  policy_template "#{node['secedit']['template']['location']}\\Create a token object.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Create global objects' do
  policy_template "#{node['secedit']['template']['location']}\\Create global objects.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Create permanent shared objects' do
  policy_template "#{node['secedit']['template']['location']}\\Create permanent shared objects.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Create symbolic links' do
  policy_template "#{node['secedit']['template']['location']}\\Create symbolic links.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Debug programs' do
  policy_template "#{node['secedit']['template']['location']}\\Debug programs.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Deny access to this computer from the network' do
  policy_template "#{node['secedit']['template']['location']}\\Deny access to this computer from the network.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Deny log on as a batch job' do
  policy_template "#{node['secedit']['template']['location']}\\Deny log on as a batch job.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Deny log on as a service' do
  policy_template "#{node['secedit']['template']['location']}\\Deny log on as a service.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Deny log on locally' do
  policy_template "#{node['secedit']['template']['location']}\\Deny log on locally.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Deny log on through Remote Desktop Services' do
  policy_template "#{node['secedit']['template']['location']}\\Deny log on through Remote Desktop Services.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Enable computer and user accounts to be trusted for delegation' do
  policy_template "#{node['secedit']['template']['location']}\\Enable computer and user accounts to be trusted for delegation.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Force shutdown from a remote system' do
  policy_template "#{node['secedit']['template']['location']}\\Force shutdown from a remote system.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Generate security audits' do
  policy_template "#{node['secedit']['template']['location']}\\Generate security audits.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Impersonate a client after authenticatio' do
  policy_template "#{node['secedit']['template']['location']}\\Impersonate a client after authenticatio.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Increase scheduling priority' do
  policy_template "#{node['secedit']['template']['location']}\\Increase scheduling priority.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Load and unload device drivers' do
  policy_template "#{node['secedit']['template']['location']}\\Load and unload device drivers.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Lock pages in memory' do
  policy_template "#{node['secedit']['template']['location']}\\Lock pages in memory.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Log on as a batch job' do
  policy_template "#{node['secedit']['template']['location']}\\Log on as a batch job.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Log on as a service' do
  policy_template "#{node['secedit']['template']['location']}\\Log on as a service.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Manage auditing and security log' do
  policy_template "#{node['secedit']['template']['location']}\\Manage auditing and security log.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Modify an object label' do
  policy_template "#{node['secedit']['template']['location']}\\Modify an object label.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Modify firmware environment values' do
  policy_template "#{node['secedit']['template']['location']}\\Modify firmware environment values.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Perform volume maintenance tasks' do
  policy_template "#{node['secedit']['template']['location']}\\Perform volume maintenance tasks.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Profile single process' do
  policy_template "#{node['secedit']['template']['location']}\\Profile single process.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Profile system performance' do
  policy_template "#{node['secedit']['template']['location']}\\Profile system performance.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Replace a process level token' do
  policy_template "#{node['secedit']['template']['location']}\\Replace a process level token.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Restore files and directories' do
  policy_template "#{node['secedit']['template']['location']}\\Restore files and directories.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Shut down the system' do
  policy_template "#{node['secedit']['template']['location']}\\Shut down the system.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Take ownership of files or other objects' do
  policy_template "#{node['secedit']['template']['location']}\\Take ownership of files or other objects.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
