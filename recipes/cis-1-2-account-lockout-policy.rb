...
security_policy 'Account lockout duration' do
  policy template "#{node['secedit']['template']['location']\\Account lockout duration.inf}"
...
security_policy 'Account lockout threshold' do
  policy template "#{node['secedit']['template']['location']\\Account lockout threshold.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Reset account lockout counter after' do
  policy template "#{node['secedit']['template']['location']\\Reset account lockout counter after.inf}"
  database "#{node['secedit']['database']['location']}\\#{['secedit']['database']['name']}"
  action :configure
end
