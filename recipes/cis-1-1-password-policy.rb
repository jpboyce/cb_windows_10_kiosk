...
security_policy 'Enforce password history' do
  policy_template "#{node['secedit']['template']['location']}\\Enforce password history.inf"
...
security_policy 'Maximum password age' do
  policy_template "#{node['secedit']['template']['location']}\\Maximum password age.inf"
  database "#{node['secedit']['database']['location']}\\#{node['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Minimum password age' do
  policy_template "#{node['secedit']['template']['location']}\\Minimum password age.inf"
  database "#{node['secedit']['database']['location']}\\#{node['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Minimum password length' do
  policy_template "#{node['secedit']['template']['location']}\\Minimum password length.inf"
  database "#{node['secedit']['database']['location']}\\#{node['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Password must meet complexity requirements' do
  policy_template "#{node['secedit']['template']['location']}\\Password must meet complexity requirements.inf"
  database "#{node['secedit']['database']['location']}\\#{node['secedit']['database']['name']}"
  action :configure
end
...
security_policy 'Store passwords using reversible encryption' do
  policy_template "#{node['secedit']['template']['location']}\\Store passwords using reversible encryptiony.inf"
  database "#{node['secedit']['database']['location']}\\#{node['secedit']['database']['name']}"
  action :configure
end
